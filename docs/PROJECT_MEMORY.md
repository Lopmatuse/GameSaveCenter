...
> 0.6.53 将任务/概览状态和维护诊断等级改为主题适配的可读状态。0.6.52 将修改器列表的自动启动字段改为可读策略文本。0.6.51 将存档历史的布尔字段改为可读状态胶囊，并让 Worker 初始化日志写出程序集版本。用户提供的崩溃日志实际加载的是 0.6.22；排查 900+ 游戏库时必须先确认 Playnite 与 `worker-launch.log` 都报告 0.6.53。
