...
> 0.6.34 在 0.6.33 的大型库启动闸门上继续收紧 Dashboard 首次打开路径：已有 SQLite 游戏缓存时先显示缓存并等待 60 秒空闲窗口，空缓存使用 10 秒延迟后才开始整库同步；Dashboard 未打开前也不启动任务通知长轮询，避免 900+ 游戏启动阶段与其他 Ludusavi 集成争用；用户手动刷新会取消延迟并明确执行同步，视图卸载会取消未开始的同步。
