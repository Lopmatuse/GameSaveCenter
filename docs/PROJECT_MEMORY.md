...
> 0.6.33 在 0.6.32 的大型库启动闸门上继续收紧 Dashboard 首次打开路径：已有 SQLite 游戏缓存时先显示缓存并等待 60 秒空闲窗口，空缓存使用 10 秒延迟后才开始整库同步；用户手动刷新会取消延迟并明确执行同步，视图卸载会取消未开始的同步，避免 900+ 游戏激活侧栏时立刻与其他 Ludusavi 集成争用。
