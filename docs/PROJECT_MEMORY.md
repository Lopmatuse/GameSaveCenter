...
- 本轮自动化证据：`validate-source.py` 通过；UI Skill 静态审查为 0 错误（仍有项目和 `.tmp` 副本的提示，不能忽略）；Release 构建 0 警告/0 错误；Core 13、Worker 21、Playnite 32，共 66 测试通过；PEXT 包检查与 Worker `0.6.22.0` smoke 通过。
