...
更新时间：2026-07-27
当前版本：`0.4.0-development-preview`
...
1. 当前交付环境没有可用 .NET SDK/MSBuild，因此最新 0.4.0 改动只能做结构校验；早期版本已由用户在 Windows 完成 restore/build/test/package 和 Playnite 加载。
...
4. 公共截图目录已接入文件名和无重叠游戏会话时间窗口归类；未识别收件箱和复杂并发会话仍需完善。
...
4. 实现 Worker 主动任务事件推送、公共截图会话归类和远端设备摘要；回归并调优现有前后快照候选链路。
