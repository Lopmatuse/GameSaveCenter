...
更新时间：2026-08-05
当前版本：`0.6.55-development-preview`

> 0.6.55 将总览“需关注”指标改为可访问的圆角导航卡。0.6.54 让 Worker 启动日志同时写出期望程序集版本，便于和初始化日志核对。0.6.53 将任务/概览状态和维护诊断等级改为主题适配的可读状态。0.6.52 将修改器列表的自动启动字段改为可读策略文本。0.6.51 将存档历史的布尔字段改为可读状态胶囊，并让 Worker 初始化日志写出程序集版本。用户提供的崩溃日志实际加载的是 0.6.22；排查 900+ 游戏库时必须先确认 Playnite 与 `worker-launch.log` 都报告 0.6.55。
