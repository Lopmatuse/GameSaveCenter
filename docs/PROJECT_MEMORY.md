...
更新时间：2026-08-05
当前版本：`0.6.38-development-preview`

> 0.6.38 针对 900+ 游戏库继续收口启动负载：同步只向 SQLite 写入描述发生变化的游戏，描述输入变化会清除旧匹配并重新排队；Worker 启动等待使用真实 30 秒截止而不是可能累积数分钟的固定探针；Dashboard 首次缓存请求快速失败并后台重试，任务事件连接延迟 60 秒且卸载时安全取消。当前自动化证据为 Core 13、Worker 23、Playnite 89，共 125 项通过；真实 Playnite 仍待用户侧验证。
