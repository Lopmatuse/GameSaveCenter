...
更新时间：2026-08-05
当前版本：`0.6.33-development-preview`

- [x] 0.6.33：大型库 Dashboard 首次打开先读持久化缓存，已有缓存延迟 60 秒、空缓存延迟 10 秒再释放整库同步；手动刷新和视图卸载具备取消/接管语义。
...
- [ ] 仍需在 900+ 游戏的真实 Playnite 环境确认扩展版本为 0.6.33，并验证旧 Worker 不会被误杀。
...
- [ ] 仍需在真实 Playnite 中确认加载的是 `GameSaveCenter 0.6.33`，并与独立 `LudusaviPlaynite` 扩展分开测量。
