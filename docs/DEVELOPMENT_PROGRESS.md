...
> Windows 真机已完成编译、单元测试、Worker 发布、PEXT 打包和 Playnite 加载。0.4.0 新增代码在本执行环境只完成静态校验，仍需在 Windows 重新 build/test/package 并按 `WINDOWS_TEST_PLAN.md` 回归。
...
| Xbox/Game Bar 媒体同步 | 🧪 | 公共 Captures 目录支持文件名匹配和无重叠会话时间窗口归类；待真机完善 |
...
| 公共截图会话归类 | 🧪 | 名称归类与无重叠游戏会话时间窗口均已接入；重叠会话自动放弃时间推断，待 Windows 数据验证 |
