...
更新时间：2026-08-05
目标版本：`0.6.32-development-preview`

0.6.32 延续全工作区滚动收口，并将大型库同步静默窗口、交互启动闸门和任务通知首次连接分开调度；Worker 在忙碌时必须先等待恢复，Dashboard 首屏必须优先使用持久化缓存，打开侧栏时加入正在运行的同步任务，所有工作区表格在矮窗口必须保留页面级滚动和有限视口；真实 Playnite 验证仍未在本环境执行。
...
| GSC-012 | 900+ 游戏库启动期间 Ludusavi 与其他插件争用 | 0.6.32 增加未打开面板前的整库同步闸门，并保留 0.6.31 的启动空闲窗口、同步合并、Worker 忙碌恢复、大型库通知延迟和侧栏同步去重，待真机验证 | 确认 `playnite.log` 加载 0.6.32；对比独立 LudusaviPlaynite 关闭/开启时的启动时间、Worker 请求数量和 60 秒内的命名管道超时 |
