...
更新时间：2026-08-05
目标版本：`0.6.41-development-preview`

0.6.41 延续全工作区滚动收口，并修正共享上下文操作在 disabled 时被折叠导致的布局跳动；0.6.40 的低高度信息保留和主题兜底、0.6.39 的页面滚动与表格密度，以及 0.6.38 的大型库启动保护继续保留。真实 Playnite 验证仍未在本环境执行。
