...
更新时间：2026-08-05
目标版本：`0.6.38-development-preview`

0.6.38 延续全工作区滚动收口，并修复大型库同步无变化重复写入、匹配输入变化后旧结果残留、Worker 启动等待可能累积数分钟、首次 Dashboard IPC 长时间阻塞和任务事件取消未观察等问题；后台 Ludusavi 匹配限制为已安装/近期游玩优先且每轮最多 64 个，Worker 单实例互斥和 45 秒同路径恢复窗口避免重复进程与误杀；真实 Playnite 验证仍未在本环境执行。
