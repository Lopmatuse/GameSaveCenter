...
更新时间：2026-08-05
目标版本：`0.6.29-development-preview`

0.6.29 新增全工作区滚动收口：Worker 在忙碌时必须先等待恢复，Dashboard 首屏必须优先使用持久化缓存，所有工作区表格在矮窗口必须保留页面级滚动和有限视口；真实 Playnite 验证仍未在本环境执行。
...
| GSC-012 | 900+ 游戏库启动期间 Ludusavi 与其他插件争用 | 0.6.29 已增加启动空闲窗口、同步合并和 Worker 忙碌恢复，待真机验证 | 确认 `playnite.log` 加载 0.6.29；对比独立 LudusaviPlaynite 关闭/开启时的启动时间和 Worker 请求数量 |
