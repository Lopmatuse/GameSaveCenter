...
| GSC-078 | Settings 引用 Dashboard 局部按钮资源、输入错误填充资源缺失，且数值静态门禁漏检嵌套路径 | 已确认待 UI-001 修复 | 2026-08-01 使用 Codex 附带 Python 重现源码门禁退出码 1；必须将跨视图样式迁入共享令牌并修复嵌套绑定路径检查，再完成 Playnite 回归 |
