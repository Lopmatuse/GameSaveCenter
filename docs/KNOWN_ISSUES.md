...
更新时间：2026-08-05
目标版本：`0.6.35-development-preview`

0.6.35 延续全工作区滚动收口，并将大型库 Worker 启动、同步静默窗口、交互启动闸门、首次 Dashboard 缓存延迟和任务通知首次连接分开调度；Worker 首次进程扫描只建立基线，Dashboard 首屏必须优先显示持久化缓存，所有工作区表格在矮窗口必须保留页面级滚动和有限视口；真实 Playnite 验证仍未在本环境执行。
...
| GSC-012 | 900+ 游戏库启动期间 Ludusavi 与其他插件争用 | 0.6.35 进一步延迟大型库 Worker 启动；打开面板或游戏启动才按需启动，整库同步仍有缓存优先和后台分批闸门，待真机验证 | 确认 `playnite.log` 加载 0.6.35；比较启动阶段是否没有 GameSaveCenter Worker、打开面板后是否只启动一个 Worker，并对比独立 LudusaviPlaynite 开启/关闭时 CPU、磁盘和命名管道超时 |
