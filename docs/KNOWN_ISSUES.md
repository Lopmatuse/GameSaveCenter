...
更新时间：2026-08-05
目标版本：`0.6.39-development-preview`

0.6.39 延续全工作区滚动收口：Settings 与侧栏使用共享页面滚动，低高度保留摘要而不静默隐藏，表格视口提高以显示更多行；0.6.38 的大型库启动、缓存和 Worker 冷启动保护继续保留。真实 Playnite 验证仍未在本环境执行。
