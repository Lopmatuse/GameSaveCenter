...
更新时间：2026-08-05
目标版本：`0.6.36-development-preview`

0.6.36 延续全工作区滚动收口，并将大型库 Worker 启动、同步静默窗口、交互启动闸门、首次 Dashboard 缓存延迟和任务通知首次连接分开调度；后台 Ludusavi 匹配限制为已安装/近期游玩优先且每轮最多 64 个，Worker 单实例互斥和 45 秒同路径恢复窗口避免重复进程与误杀；真实 Playnite 验证仍未在本环境执行。
...
| GSC-012 | 900+ 游戏库启动期间 Ludusavi 与其他插件争用 | 0.6.36 延迟大型库 Worker 启动；后台匹配只处理已安装/近期游玩且每轮最多 64 个，单实例互斥避免重复 Worker；待真机验证 | 确认 `playnite.log` 加载 0.6.36；比较启动阶段是否没有 GameSaveCenter Worker、打开面板后是否只启动一个 Worker，并对比独立 LudusaviPlaynite 开启/关闭时 CPU、磁盘和命名管道超时 |
