...
更新时间：2026-08-05
目标版本：`0.6.33-development-preview`

0.6.33 延续全工作区滚动收口，并将大型库同步静默窗口、交互启动闸门和首次 Dashboard 缓存延迟、任务通知首次连接分开调度；Worker 在忙碌时必须先等待恢复，Dashboard 首屏必须优先显示持久化缓存，所有工作区表格在矮窗口必须保留页面级滚动和有限视口；真实 Playnite 验证仍未在本环境执行。
...
| GSC-012 | 900+ 游戏库启动期间 Ludusavi 与其他插件争用 | 0.6.33 增加未打开面板前的整库同步闸门与首次 Dashboard 缓存延迟，并保留 0.6.31 的启动空闲窗口、同步合并、Worker 忙碌恢复、大型库通知延迟和侧栏同步去重，待真机验证 | 确认 `playnite.log` 加载 0.6.33；对比独立 LudusaviPlaynite 关闭/开启时的启动时间、Worker 请求数量和 60 秒内的命名管道超时 |
